...
function [zz,Az,Vz,tt,Qst,Kzt,Tzt,Czt,Szt,Pzt,Chlzt,PPzt,DOPzt,DOCzt,Qzt_sed,lambdazt,...
         P3zt_sed,P3zt_sed_sc,His,DoF,DoM,MixStat,Wt,Phys_par] = ...
         solvemodel_v12_2017(M_start,M_stop,Initfile,Initsheet,Inputfile,Inputsheet,Parafile,Parasheet,...
         Kz_ak,Kz_ak_ice,Kz_N0,C_shelter,alb_melt_ice,alb_melt_snow,one_run_only,varargin)
...
%else
 %Read input data
 disp([Parafile,'  ',Parasheet]);
 disp([Initfile,'  ',Initsheet]);
 disp([Inputfile,'  ',Inputsheet]);
 %Kz_N0
  [In_Z,In_Az,tt,In_Tz,In_Cz,In_Sz,In_TPz,In_DOPz,In_Chlz,In_DOCz,In_TPz_sed,In_Chlz_sed,In_FIM,Ice0,Wt,Inflw,...
         Phys_par,Phys_par_range,Phys_par_names,Bio_par,Bio_par_range,Bio_par_names]...
...
if one_run_only == 1    % only one run, read para from para file  
    ...
    lat = Phys_par(6); %latitude (decimal degrees)
    lon = Phys_par(7); %longitude (decimal degrees)
    %**alb_melt_ice = alb_melt_ice;
    %**alb_melt_snow = alb_melt_snow;
end
