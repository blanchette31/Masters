...
function plot_mod_vs_obs_CRO_20152019_singlerun_v1(figno,tt,Tzt,Kz_ak,Kz_ak_ice,Kz_N0,C_shelter,alb_melt_ice,alb_melt_snow)

clf;

% *** to be specified before runnning ***

%obstemfile = 'Croche_temperature_daily_all_depths.txt'; % file temperature observations
...
mintime = [2015  6  1  0  0  0];
maxtime = [2019 12  1  0  0  0];
mintem =  0.0;
maxtem =  30.0;

% **********

%tem02m = temdata(:,6);  % temp. at 2.0 m
%tem04m = temdata(:,7);  % temp. at 4.0 m
%tem06m = temdata(:,8);
%tem10m = temdata(:,10);
%tem16m = temdata(:,13);
%tem26m = temdata(:,16);
...
% *** subplot no.1 - temperature at 1.0 m *********************************
subplot(6,1,1)
%axes('Position',[0.1,0.5,0.8,0.3],'Box','on');
%hold on;

%plot(dates2,tem02m,'color',color2,'linewidth',w4blue);
%hold on;
%plot(tt,mod_temp(2,:),'color',color4,'linewidth',w4blue);
...
%dataobs = tem02m;
%%size(dataobs)
%datamod = (mod_temp(1,:)+mod_temp(2,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 02 m ' num2str(rmse_obsmod)]);

%xtext2 = mintime + (maxtime-mintime)*x4rmse;
%%ytext2 = 6;
%textrsme = ['rsme = ' num2str(rmse_obsmod,'%4.2f')];
%text(xtext2,ytext2,textrsme,'fontsize',10,'color','k');

% *** subplot no.2 - temperature at 2.0 m *********************************
subplot(6,1,2)

%plot(dates2,tem04m,'color',color2,'linewidth',w4blue);
%hold on;
...
%dataobs = tem04m;
%%size(dataobs)
%datamod = (mod_temp(4,:)+mod_temp(5,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 04 m ' num2str(rmse_obsmod)]);
...
% *** subplot no.3 - temperature at 4.0 m *********************************
subplot(6,1,3)

%plot(dates2,tem06m,'color',color2,'linewidth',w4blue);
%hold on;
...
%dataobs = tem06m;
%%size(dataobs)
%datamod = (mod_temp(6,:)+mod_temp(7,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 06 m ' num2str(rmse_obsmod)]);
...
%plot(dates2,tem10m,'color',color2,'linewidth',w4blue);
%hold on;
%plot(tt,mod_temp(8,:),'color',color4,'linewidth',w4blue);
...
%dataobs = tem10m;
%%size(dataobs)
%datamod = (mod_temp(10,:)+mod_temp(11,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 10 m ' num2str(rmse_obsmod)]);
...
%plot(dates2,tem16m,'color',color2,'linewidth',w4blue);
%hold on;
...
%dataobs = tem16m;
%%size(dataobs)
%datamod = (mod_temp(16,:)+mod_temp(17,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 16 m ' num2str(rmse_obsmod)]);
...
%plot(dates2,tem26m,'color',color2,'linewidth',w4blue);
%hold on;
%plot(tt,mod_temp(14,:),'color',color4,'linewidth',w4blue);
...
%dataobs = tem26m;
%%size(dataobs)
%datamod = (mod_temp(26,:)+mod_temp(27,:))/2;
%datamod = datamod(1:1084); datamod = datamod';
%%size(datamod)

%rmse_obsmod = sqrt(immse(dataobs,datamod));
%disp(' ');
%disp(['root-mean-square error at 26 m ' num2str(rmse_obsmod)]);
