...
function plot_mod_vs_obs_CRO_2022_singlerun_v1(figno,m_start,m_stop,tt,Tzt,Kz_ak,Kz_ak_ice,Kz_N0,C_shelter,alb_melt_ice,alb_melt_snow,swa_b0,swa_b1)

clf;

% *** to be specified before runnning ***

obstemfile = 'obs_temp_2022_wide.txt'; % file temperature observations
...
dep1txt = '0.65 m';dep2txt = '1.73 m';dep3txt = '3.73 m';dep4txt = '5.58 m';dep5txt = '7.53 m';dep6txt = '10.53 m';

mintime = [2022  5  1  0  0  0];
...
tem0p5m = (temdata(:,5) + temdata(:,6)) / 2;  % temp. at 0.65 m, layer 1
tem1p0m = temdata(:,8);  % temp. at 1.73 m, layer 2
tem4p5m = temdata(:,10); % temp. at 3.73 m, layer 4
tem6p5m = temdata(:,12); % temp at 5.58 m, layer 6
tem8p5m = temdata(:,14); % temp at 7.53 m layer 8
tem10p5m = temdata(:,16); % temp at 10.53, layer 11
...
for k = 1:4:100
    ...
    ticks(k+1)=datenum(year,4,1,0,0,0);
    ticks(k+2)=datenum(year,7,1,0,0,0);
    ticks(k+3)=datenum(year,10,1,0,0,0);
end
...
% *** subplot no.1 - temperature at 0.65 m *********************************
subplot(6,1,1)
...
% *** subplot no.2 - temperature at 1.73 m *********************************
subplot(6,1,2)
...
dataobs = tem1p0m;
datamod = (mod_temp(2,:)); datamod = datamod';
datamod(indexnan) = [];
...
% *** subplot no.3 - temperature at 3.73 m *********************************
subplot(6,1,3)
...
dataobs = tem4p5m;
datamod = mod_temp(4,:); datamod = datamod';
datamod(indexnan) = [];
...
% *** subplot no.4 - temperature at 5.58 m *********************************
subplot(6,1,4)
...
dataobs = tem8p5m;
datamod = mod_temp(8,:); datamod = datamod';
datamod(indexnan) = [];
