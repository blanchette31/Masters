...
function plot_mod_vs_obs_CRO_2015_singlerun_v2(figno,m_start,m_stop,tt,Tzt,Kz_ak,Kz_ak_ice,Kz_N0,C_shelter,alb_melt_ice,alb_melt_snow,swa_b0,swa_b1)

clf;

% *** to be specified before runnning ***

obstemfile = 'CRO_obstemp_daily_alldepths_2015.txt'; % file temperature observations
temdata = readmatrix(obstemfile,'NumHeaderLines',1); % temperature data

% Specify the output CSV file path
outputFile = 'temperature_data.csv';

% Write the data to a CSV file
writematrix(temdata, outputFile);
...
dep1txt = '1.7 m';dep2txt = '4 m';dep3txt = '5 m';dep4txt = '7 m';dep5txt = '8 m';dep6txt = '10 m';

mintime = [2015  5  1  0  0  0];
maxtime = [2015 12 31  0  0  0];
mintem =  0.0;
maxtem =  30.0;
...
tem1p7m = temdata(:,5) ;  % temp. at 0.5 m
tem4p0m = temdata(:,6);  % temp. at 1.0 m
tem5p0m = temdata(:,7);
tem6p0m = temdata(:,8);
tem8p0m = temdata(:,10);
tem10p0m = temdata(:,12);
...
plot(tt,(mod_temp(2,:)+mod_temp(3,:))/2,'color',color4,'linewidth',w4blue);
hold on;
plot(dates2,tem1p7m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
%size(dates2)
%size(tem02m)
...
disp(' ');
disp(['root-mean-square error at 1.7 m ' num2str(rmse_obsmod)]);
...
hold on;
plot(dates2,tem4p0m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
...
rmse_obsmod = sqrt(immse(dataobs,datamod));
disp(' ');
disp(['root-mean-square error at 4.0 m ' num2str(rmse_obsmod)]);
...
% *** subplot no.3 - temperature at 5.0 m *********************************
subplot(6,1,3)

%plot(dates2,tem4p5m,'color',color2,'linewidth',w4blue);
plot(tt,mod_temp(6,:),'color',color4,'linewidth',w4blue);
%plot(tt,(mod_temp(5,:))/2,'color',color4,'linewidth',w4blue);
hold on;
plot(dates2,tem5p0m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
...
dataobs = tem5p0m;
datamod = mod_temp(6,:); datamod = datamod';
datamod(indexnan) = [];

rmse_obsmod = sqrt(immse(dataobs,datamod));
disp(' ');
disp(['root-mean-square error at 5.0 m ' num2str(rmse_obsmod)]);
...
% *** subplot no.4 - temperature at 6.0 m *********************************
subplot(6,1,4)

%plot(dates2,tem6p0m,'color',color2,'linewidth',w4blue);
plot(tt,mod_temp(7,:),'color',color4,'linewidth',w4blue);
%plot(tt,(mod_temp(6,:)+mod_temp(7,:))/2,'color',color4,'linewidth',w4blue);
hold on;
plot(dates2,tem6p0m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
...
rmse_obsmod = sqrt(immse(dataobs,datamod));
disp(' ');
disp(['root-mean-square error at 6.0 m ' num2str(rmse_obsmod)]);
...
% *** subplot no.5 - temperature at 8.0 m ********************************
subplot(6,1,5)
...
hold on;
plot(dates2,tem8p0m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
...
rmse_obsmod = sqrt(immse(dataobs,datamod));
disp(' ');
disp(['root-mean-square error at 8.0 m ' num2str(rmse_obsmod)]);
...
% *** subplot no.6 - temperature at 10.0 m ********************************
subplot(6,1,6)

%plot(dates2,tem10p0m,'color',color2,'linewidth',w4blue);
plot(tt,mod_temp(11,:),'color',color4,'linewidth',w4blue);
%plot(tt,(mod_temp(10,:)+mod_temp(11,:))/2,'color',color4,'linewidth',w4blue);
hold on;
plot(dates2,tem10p0m,'o','markerfacecolor',color2,'markeredgecolor',color2,'markersize',1.5);
hold on;
...
rmse_obsmod = sqrt(immse(dataobs,datamod));
disp(' ');
disp(['root-mean-square error at 10.0 m ' num2str(rmse_obsmod)]);
...
text_para1 = ['Kz ak: ',num2str(Kz_ak),'      Kz ak ice: ',num2str(Kz_ak_ice),'      Kz N0: ',num2str(Kz_N0), ...
    ...
    '      swa b0: ',num2str(swa_b0),'      swa b1: ',num2str(swa_b1)];
