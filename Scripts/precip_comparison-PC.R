...
df <- merge(nasa[,c("year", "doy","rain_nasa")],
            df,
            by = c("year", "doy"), all = TRUE)
...
ggplot(df, aes(y = rain_nasa, x = rain_sth, color = as.factor(year))) +
  ...
  geom_abline(intercept = 0, slope = 1, lty = 2) +
  scale_color_brewer(palette = 'Set1', name = '') +
  labs(x = 'Rain NASA (mm)', y = 'Rain Saint-Hippolyte (mm)') +
  theme_minimal()
ggsave('../export/precip_sbl_vs_sth_scatterplot.pdf', device = 'pdf', width = 5, height = 4)
...
ggplot(df) +
  ...
  geom_bar(aes(x= doy, y = rain_nasa), fill ="yellow", stat = "identity", alpha = 0.7) + 
  labs(y = 'Rain (mm)') +
  facet_wrap(~year) +
  theme_minimal()
