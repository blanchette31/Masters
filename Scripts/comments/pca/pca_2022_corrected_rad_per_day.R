...
# Load data
df <- read.csv("Data/Processed/comments/comp_years_2022_rad_per_day.csv", header = TRUE, row.names = 1)

# Remove columns that won't be used for PCA
df_pca <- df %>% 
  select(-c(g_C_CO2_per_day, max_wind_fall, sum_wind_fall, cum_temp_sum, avg_temp_sum, offset, width, stretch, departure_slp_abs, cum_rad_sum,cum_rad_fall))
...
ggsave("Data/Figures/comments/pca_2022_corrected_rad_fall_per_day.jpg", p2, dpi = 300, height = 6, width = 10, units = "in")
...
ggsave("Data/Figures/pca_2022_corrected_rad_fall_per_day_no_labels.jpg", p3, dpi = 300, height = 6, width = 10, units = "in")
