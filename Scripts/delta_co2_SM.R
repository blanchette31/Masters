...
#load data
df <- read.csv(here("Data/Processed/lake/delta_co2_SM.csv"), header = TRUE)
...
p1 <-   ggplot(doy, aes(x = as.numeric(doy), y = mean))+
  ...
  xlab("Day of year")+
  theme_minimal()
ggsave("Data/export/delta_co2_SM_full_year.png", p1, dpi = 300)
...
ylab(expression(paste("Average ", Delta, "CO"[2]," (", mu, "mol/L)")))+
  xlab("Day of year")+
  theme_minimal()
ggsave("Data/export/delta_co2_SM_study_period.png", p2, dpi = 300)
