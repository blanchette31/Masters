...
# Function to fit linear models and check residuals for each year
check_residuals <- function(df, response, predictor) {
  models <- df %>%
    group_by(year) %>%
    do(model = lm(as.formula(paste(response, "~", predictor)), data = .))
...
co2_residuals_plots <- check_residuals(lf, "co2", "doy")

# Check residuals for O2
o2_residuals_plots <- check_residuals(lf, "o2", "doy")
