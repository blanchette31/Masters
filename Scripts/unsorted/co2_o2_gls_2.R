...
# Check residuals and metrics for CO2 without AR(1)
...
# Check residuals and metrics for O2 without AR(1)
...
combined_metrics <- bind_rows(
  co2_gls_no_ar1$metrics %>% mutate(model = "CO2_No_AR1"),
  co2_gls_ar1$metrics %>% mutate(model = "CO2_AR1"),
  o2_gls_no_ar1$metrics %>% mutate(model = "O2_No_AR1"),
