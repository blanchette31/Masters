...
for (yr in years) {
  ...
  co2_range <- range(df_june_year$co2, na.rm = TRUE)
  o2_range <- range(df_june_year$o2, na.rm = TRUE)
  ymin <- min(co2_range[1], o2_range[1])
  ymax <- max(co2_range[2], o2_range[2])
  
  p <- ggplot(df_june_year, aes(x = doy, y = co2)) + 
    geom_line(linewidth = 1.3) +
    geom_line(aes(y = o2), linewidth = 1.3) +
    geom_rect(aes(xmin = doy_start, xmax = doy_end, ymin = ymin, ymax = ymax), 
